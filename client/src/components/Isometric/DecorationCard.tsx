...
export function DecorationCard({ decoration }: DecorationCardProps) {
  ...
  const handleCopySVG = async () => {
    try {
      await navigator.clipboard.writeText(decoration.svg);
      setCopied(true);
  ...
  const handleDownloadSVG = () => {
    const blob = new Blob([decoration.svg], { type: 'image/svg+xml' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.download = `${decoration.id}.svg`;
    link.href = url;
    link.click();
  ...
  const handleDownloadPNG = (scale: number) => {
    ...
    canvas.width = decoration.width * scale;
    canvas.height = decoration.height * scale;

    const img = new Image();
    const blob = new Blob([decoration.svg], { type: 'image/svg+xml;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    ...
    img.onload = () => {
      ...
      link.download = `${decoration.id}@${scale}x.png`;
      link.click();
    };
  ...
  return (
    <div className="bg-[#1e1e1e] border border-[#333] rounded-lg overflow-hidden hover:border-[#444] hover:shadow-lg transition-all group">
      ...
      <div className="p-3 border-t border-[#333]">
        <h3 className="text-xs font-medium text-white mb-2">{decoration.name}</h3>
